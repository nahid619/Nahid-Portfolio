// components/portfolio/AllExperiencesClient.js
// "use client" — handles modal open/close, hover effects
// Receives experiences as prop from server page — no fetching inside
...
function formatPeriod(startDate, endDate, isCurrent) {
  ...
  const rem   = months % 12;
  const dur   = years > 0
    ? `${years} year${years !== 1 ? "s" : ""} ${rem > 0 ? rem + " mo" : ""}`.trim()
    : `${months} months`;
  return { period: `${start} – ${end}`, duration: dur };
}

export default function AllExperiencesClient({ experiences = [] }) {
          ...
          return (
            <div
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow   = "0 6px 24px rgba(5,146,18,0.15)";
              }}
              onMouseLeave={e => {
            ...
            >
              ...
              <div style={{ textAlign: "left" }}>
                <h3 style={{ color: "white", fontSize: "1rem", fontWeight: 700, margin: "0 0 3px" }}>
                  {exp.role}
                </h3>
                ...
                {exp.skills?.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
                    ...
                    {exp.skills.length > 5 && (
                      <TechBadge label={`+${exp.skills.length - 5}`} variant="outline" />
                    )}
                  </div>
                )}
              </div>
              ...
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ color: "#9BEC00", fontSize: "0.875rem", fontWeight: 700 }}>
                  {duration}
                </div>
                {exp.isCurrent && (
                  <span style={{
                ...
                <div style={{ color: "#bcc4ba", fontSize: "0.75rem", marginTop: "6px" }}>
                  View details →
                </div>
              </div>
            </div>
